...
def delist(data: Any) -> Any:
    ...
    """
    if isinstance(data, collections.abc.Iterable) and not isinstance(data, str) and len(data) == 1:
        return data[0]
    return data


def embed(value: Any, mimetype: str | None = None, encoding: str | None = None) -> tuple[bytes, str] | tuple[str, str | Any] | tuple[Any, str | Any]:
    """
...
def guess_type(url: str | Path, supported: list[str] | tuple[str]) -> tuple[str, str]:
    """
    ...
    # netCDF
    if mime == "application/x-netcdf" and "dodsC" in str(url) and "application/x-ogc-dods" in supported:
