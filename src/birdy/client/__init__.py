...
The :py:class:`WPSClient` class aims to make working with WPS servers easy, even without any prior knowledge of WPS.

Calling the :py:class:`WPSClient` class creates an instance whose methods call WPS processes.
...
returned as a :py:class:`WPSExecution` instance, which includes information about the job status, the progress percentage,
...
.. code-block:: python
    ...
    # Just says a friendly Hello. Returns a literal string output with Hello plus the inputted name.
    #
    # Parameters
    # ----------
    # name : string
