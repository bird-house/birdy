...
def filter_case_insensitive(names: str | list[str], complete_list: list[str]) -> tuple[list[str], list[str]]:
    """
...
def build_wps_client_doc(wps: WebProcessingService, processes: dict[str, Process]) -> str:
    """
    ...
    for process_name, process in list(processes.items()):
        ...
        description = "{name}\n    {abstract}".format(name=sanitized_name, abstract=process.abstract or "(No description)")
