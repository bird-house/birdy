...
class WPSClient:
    ...
    def _get_process_description(self, processes=None, xml=None):
        ...
        process_names, missing = utils.filter_case_insensitive(processes, all_wps_processes)
    ...
    def _method_factory(self, pid: str) -> Callable:
        ...
        defaults = []
        # Set process inputs
        for _inpt in required_inputs_first:
            input_names.append(sanitize(_inpt.identifier))
            if _inpt.minOccurs == 0 or _inpt.defaultValue is not None:
                default = _inpt.defaultValue if _inpt.dataType != "ComplexData" else None
                defaults.append(utils.from_owslib(default, _inpt.dataType))
        ...
        if hasattr(process, "dataInputs"):
            self._inputs[pid] = OrderedDict((i.identifier, i) for i in process.dataInputs)

        self._outputs[pid] = {}
        if hasattr(process, "processOutputs"):
            self._outputs[pid] = OrderedDict((o.identifier, o) for o in process.processOutputs)
    ...
    def _execute(self, pid, **kwargs):
        ...
        if not wps_outputs:
            wps_outputs = [(o.identifier, "ComplexData" in o.dataType) for o in list(self._outputs[pid].values())]
    ...
    def _console_monitor(self, execution: WPSExecution, sleep: int = 3):
        ...
        # Intercept CTRL-C
        def sigint_handler(signum, frame):  # noqa: F841
            self.cancel()
