...
.. warning::
    ...
    It is important to be aware that any changes to files found within the ``src/birdy`` folder (with the exception of ``src/birdy/__init__.py``) will trigger the ``bump-version.yml`` workflow. Be careful not to commit changes to files in this folder when preparing a new release.
...
The new version based off of the version checked out will now be available via `pip` (`pip install birdy`).
 ...
 * If possible, configure tests within the conda-forge build CI (e.g. `imports: birdy`, `commands: pytest birdy`).
...
From the birdy source folder we can enter into the docker container, providing access to the `src/birdy` source files by linking them to the running image:

    .. code-block:: console

        sudo docker run --rm -ti -v $(pwd):/src/birdy -w /src/birdy quay.io/pypa/manylinux_2_24_x86_64 bash
...
This will then place two files in `birdy/dist/` ("birdy-1.2.3-py3-none-any.whl" and "birdy-1.2.3.tar.gz").
