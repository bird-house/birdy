...
Report bugs at https://github.com/bird-house/birdy/issues.
...
birdy could always use more documentation, whether as part of the official birdy docs, in docstrings, or even on the web in blog posts, articles, and such.

Submit Feedback
~~~~~~~~~~~~~~~

The best way to send feedback is to file an issue at https://github.com/bird-house/birdy/issues.
...
Ready to contribute? Here's how to set up ``birdy`` for local development.

#. First, clone the ``birdy`` repo locally.

    * If you are not a ``birdy`` collaborator, first fork the ``birdy`` repo on GitHub, then clone your fork locally.

        .. code-block:: console

            git clone git@github.com:your_name_here/birdy.git

    * If you are a ``birdy`` collaborator, clone the ``birdy`` repo directly.
...
#. Install your local copy into a development environment. You can create a new Anaconda development environment with:
    ...
    This installs ``birdy`` in an "editable" state, meaning that changes to the code are immediately seen by the environment. To ensure a consistent coding style, `make dev` also installs the ``pre-commit`` hooks to your local clone.
...
#. Before merging, your Pull Request will need to be based on the `main` branch of the ``birdy`` repository. If your branch is not up-to-date with the `main` branch, you can perform similar steps as above to update your branch:
...
#. Once your Pull Request has been accepted and merged to the `main` branch, several automated workflows will be triggered:
    ...
    - `ReadTheDocs` will automatically build the documentation and publish it to the `latest` branch of `birdy` documentation website.
    - If your branch is not a fork (i.e. you are a maintainer), your branch will be automatically deleted.

You will have contributed to ``birdy``!
...
#. The pull request should include tests and should aim to provide `code coverage <https://en.wikipedia.org/wiki/Code_coverage>`_ for all new lines of code. You can use the `--cov-report html --cov birdy` flags during the call to ``pytest`` to generate an HTML report and analyse the current test coverage.
...
Within the context of birdy development, AI assistance constitutess any code, code snippets, configurations, documentation, or other programmatic assets produced, suggested, or significantly modified by an artificial intelligence model, tool, or service.
...
If you are using any kind of AI assistance while contributing to birdy, **this must be disclosed in the pull request**, along with the extent to which AI assistance was used (e.g. documentation, code generation, review comments).
...
The birdy maintainers reserve the right to reject Pull Requests and contributions for many different reasons (functionality out-of-scope, code quality issues, non-adherence to code of conduct, etc.)
...
.. code-block:: console
    ...
    python -m pytest tests/test_birdy.py

You can also directly call a specific test class or test function using:

.. code-block:: console

    python -m pytest tests/test_birdy.py::TestClassName::test_function_name
...
.. code-block:: console
    ...
    python -m flake8 src/birdy tests
    validate-docstrings src/birdy/**.py
...
Please note that this project is released with a `Contributor Code of Conduct <https://github.com/bird-house/birdy/blob/main/CODE_OF_CONDUCT.md>`_.
